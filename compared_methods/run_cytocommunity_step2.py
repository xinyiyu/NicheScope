...
if __name__ == '__main__':
    ...
    ## Hyperparameters
    InputFolderName = f"{args.out_dir}/{args.data_name}/Input/"
    Image_Name = "simu"
    Num_TCN = 6
    Num_Run = 10
    Num_Epoch = 100
    ...
    LastStep_OutputFolderName = f"{args.out_dir}/{args.data_name}/Step1_Output/"
    ...
    ThisStep_OutputFolderName = f"{args.out_dir}/{args.data_name}/Step2_Output"
    ...
    while run_number <= Num_Run:  #Generate multiple independent runs for ensemble.
        ...
        for epoch in range(1, Num_Epoch+1):  #Specify the number of epoch in each independent run.
            ...
            if epoch % 100 == 0:
                print(f"Epoch: {epoch:03d} / {Num_Epoch:03d}, Train Loss: {train_loss:.4f}")
